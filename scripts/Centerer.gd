...
func layout_children_packed() -> void:
  ...
  if num % 2 == 0: # even num of children
    var starting_x = mid_point_x - (num / 2)
    for i in num:
      layout_child_packed(starting_x, i, controls[i])
  else: # odd num of children
    var width_unit = controls[0].rect_size.x
    var starting_x = mid_point_x - floor(num / 2) * width_unit
    for i in num:
      layout_child_packed(starting_x, i, controls[i])
...
func layout_child_packed(starting_x: int, i: int, child: Control) -> void:
  ...
  var child_size = Vector2(get_child(0).rect_size.x, 0)
  var child_x_offset = child_size.x / 2
  var child_pos = Vector2(starting_x * (i + 1) - child_x_offset, 0)
